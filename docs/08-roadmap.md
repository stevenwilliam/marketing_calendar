...
| M5 | Master data | company, site, site group (+ auto system group), holidays | creating a site creates its group in one transaction |
...
| M10 | Importer | CSV import, idempotency, rejections, reconciliation | re-import inserts zero |
| M11 | Reports | promotion report, target vs actual, pipe CSV | export matches the screen |
| M12 | Notifications | email on release, recipient lists | a failed send does not block approval |
...
- **Promo P&L** — discount cost against incremental sales. A promotion that
  raised revenue and destroyed margin currently looks like a success (Q23).
